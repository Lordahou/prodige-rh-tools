...
const TIME_SAVINGS: Record<string, { minutes: number; label: string }> = {
  ...
  "/newsletter":      { minutes: 60, label: "Newsletter" },
...
const modules = [
  ...
  {
    title: "Newsletter RH",
    description: "Lettre mensuelle pour vos clients DRH, enrichie par la veille tendances et vos données activité.",
    href: "/newsletter",
    badge: true,
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor">
...
export default function Home() {
  ...
  const resetRoi = () => {
    try {
      localStorage.removeItem("prodige_stats");
      localStorage.removeItem("prodige_recent");
    } catch {}
    setStats({});
    setRecent([]);
  };
          ...
          <div className="flex flex-wrap items-center gap-6">
            ...
            {totalMinutes > 0 && (
              <>
                ...
                <button
                  onClick={resetRoi}
                  className="ml-auto text-white/15 hover:text-white/40 transition-colors flex items-center gap-1"
                  title="Remettre le compteur ROI à zéro"
                >
